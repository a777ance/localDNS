...
1. **Empanel 5 jurors at once.** In a **single message**, spawn 5 concurrent
   `juror` subagents (Task tool, `subagent_type: "juror"`), each handed the exact
   same question ($ARGUMENTS) with no cross-talk — their independence is what
   makes 5 votes worth ~5. Ask each to end with a canonical `ANSWER:` line.
...
3. **Adaptive top-up (bounded).** Judge the agreement:
   ...
   - **Split** (a 3–2, or a 2–2–1 with no majority) → empanel **4 more** jurors in
     one message, re-tally over all 9, and stop. Do **not** go past 9 — a jury
     that won't converge by 9 is telling you the question is genuinely contested
     or under-specified, which is itself the finding.
