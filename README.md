...
- **Unbound** — the DNS decision point: personal queries resolve recursively with DNSSEC (no third party ever sees them); high‑volume streaming domains are forwarded to a contained Cloudflare DoH organelle (encrypted, anycast) for speed  
...
- private recursive resolution with DNSSEC for personal traffic (Unbound — no third-party resolver in the chain); streaming domains forwarded to a contained Cloudflare DoH organelle for speed
...
Pi-hole sends every upstream query to Unbound at `172.17.0.1#5335` (the Docker bridge gateway to the host) — it does no resolver selection itself. Unbound is the single decision point: personal and sensitive domains resolve recursively with no third-party resolver in the chain, while high-volume streaming domains are forwarded to the **Cloudflare endosymbiont** — a contained `cloudflared` organelle on `127.0.0.1@5053` that speaks DNS-over-HTTPS to Cloudflare's anycast edge, encrypting the forward-path from the ISP. The split lives in `unbound/streaming-forward.conf`; the organelle is `systemd/cloudflared-proxy-dns.service`. The recursive nucleus is never pointed at the organelle — that invariant is what keeps sensitive lookups off Cloudflare.
...
| `unbound/streaming-forward.conf` | Domain split: streaming → Cloudflare endosymbiont (`127.0.0.1@5053`, DoH), all else → recursive |
| `unbound/remote-control.conf` | Unix socket for `unbound-control` |
| `systemd/cloudflared-proxy-dns.service` | cloudflared DoH organelle — the Cloudflare endosymbiont for the streaming forward-path |
