...
- **docs/architecture/clear-refeed-protocol.md** — the sync → clear → refeed ritual: how to wipe a stale session and re-seed the latest CLAUDE.md losslessly. With the `SessionStart` hook (`.claude/hooks/refeed.sh`) installed, bare `/clear` runs the whole thing end-to-end (fires the §G lazy anchor first, then loads the manifest); the `.claude/commands/repull.md` slash command (`/repull`) handles the no-clear refresh — it pulls `--ff-only` on `main` before re-reading, so it never reloads stale files.
