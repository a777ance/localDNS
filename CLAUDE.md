...
- **Never more than 9 branches in a repo** — founder's standing instruction (2026-08-08),
  ...
  an `archive/*` or two, a healthy repo sits at 3–5; nine is headroom, not a target. Retire
  a branch by **archiving it first**: an octopus commit whose parents are the stale tips
  keeps every orphaned commit reachable from one ref, after which deleting the branches
  loses nothing. Never delete a `claude/*` branch that is not reachable from an `archive/*`
  branch — 226 of the original 338 held commits that existed nowhere else.
...
- **tools/check-branch-cap.py** + **tools/retire-stale-branches.sh** — **the branch cap and the one-time cleanup it followed.** The branch-per-session habit produced **338** stale `claude/*` refs across ten repos; **226 held commits that existed nowhere else**, so bulk deletion would have destroyed work. The repair was to make deletion *provably* lossless first: one octopus commit per repo whose parents are every stale tip, pushed as `archive/claude-sessions-2026-08-08`, after which every orphaned commit stays reachable from a single ref. `retire-stale-branches.sh` then deletes the 338 (run it by hand — deletion returns HTTP 403 from the agent environment, so a session cannot do it). `check-branch-cap.py` is the site that stops the habit returning: over 9 branches fails at the commit gate, except for refs already archived, which report as PENDING so the cap can be enforced before the cleanup has run. Recover anything with `git log --oneline archive/claude-sessions-* --not origin/Yggdrasil` then `git branch <name> <sha>`.
