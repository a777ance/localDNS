...
| `tools/check-promotion.py` | run directly or in CI — **the top-gate lock (the eagle)**: refuses any PR into `main` that is a full-branch merge (head is a rail, or the Yggdrasil tip is an ancestor of the head), so `main` moves only by a `promote/*` cherry-pick | `python3 tools/check-promotion.py --head <branch> --base main` |
| `tools/ratatoskr.py` | **the squirrel** — an agentic cherry-pick courier: carries *chosen* commits one rung up the ladder (feature → doom box → Yggdrasil), never into `main` (the eagle bounds it). Dry-run by default; `--carry` to cherry-pick. Courier, not judge | `python3 tools/ratatoskr.py --to Yggdrasil <sha> [--carry]` |
| `.claude/agents/ratatoskr.md` | subagent that drives `ratatoskr.py` — carries a *chosen* promotion, never invents the selection, never reaches the well | spawn the `ratatoskr` agent |
...
- **`main` — the Well of Mimir** — vetted knowledge; the stable final repo. It moves only
  by a PR that the founder approves, and **that PR is always a cherry-pick from Yggdrasil —
  a strategic, contingent selection of specific commits, never a merge of the whole
  branch.** The promotion is a branch cut from `main` carrying only the chosen cream; the
  tip of Yggdrasil is never offered wholesale. No cadence, no auto-merge: the Well fills
  when the founder decides it does, one deliberate commit at a time. This is the Bifrost
  one-way door (§H) at portfolio scale — `main` is the outermost `*`, and no inner gate may
...
- **docs/architecture/yggdrasil-bestiary.md** — **the creatures of the tree.** The
  branch-promotion ladder (§3) told as its Norse ecosystem: Mímir's well (`main`), Odin
  (the orchestration layer that learns the runes and brews the mead/product), and the four
  creatures — **Ratatoskr** the agentic cherry-pick courier (`tools/ratatoskr.py`), **the
  eagle** that chases him off the well (`tools/check-promotion.py`), **the harts** that
  nibble the leaves so the tree never overgrows (`tools/check-branch-cap.py` + doombox),
  and **Níðhöggr** at the roots (the doom drawer). A reasoning language, not a zoo to
  stock: two creatures are already-built locks, one (the squirrel) is new, and all cash out
  to "the cream rises by selection, the well is Odin's, nothing promotes itself." Companion
  to `cell-grammar.md`.
