...
| `04-user-services/ai-orchestration/.env.example` | copy to `~/llm-router/.env` (git-ignored), set `LITELLM_MASTER_KEY` (+ `ANTHROPIC_API_KEY` for overflow; `RUNPOD_API_KEY`/`RUNPOD_POD_ID`/`TAILSCALE_GPU_HOST` for the idle-stop job) | — |
| `04-user-services/ai-orchestration/runpod-idle-stop.sh` | `~/llm-router/runpod-idle-stop.sh` (+ cron `*/5 * * * *`) | `crontab -e` |
...
| Heavy DeepSeek-R1 on local CPU overheats the client | Don't run `deepseek-r1:7b`+ on a CPU — its long chain-of-thought pins every core for minutes (cooks a laptop, throttles the t630). `04-user-services/ai-orchestration/config.yaml` now ships a reasoning ladder: `local-reason` (deepseek-r1:1.5b, t630 CPU, cool) for light work and `cloud-gpu-reason` (full R1 on a rented RunPod GPU via Tailscale, spun up on demand, auto-stopped when idle by `runpod-idle-stop.sh`) for heavy work, falling over to `cloud-overflow` when the pod is off. `TAILSCALE_GPU_HOST` and the RunPod credentials are still `CHANGE_ME` — no pod has been rented yet (`docs/REMEDIATION-BOARD.md` Track 6 "Exception (2026-08-16)"). See `04-user-services/ai-orchestration/README.md` "Offload heavy reasoning to a rented GPU." |
...
- **docs/REMEDIATION-BOARD.md** — **the standing track rotation**: the six tracks that close the gap between "well-documented" and "operationally closed" (drift closure · live-only snapshots · measured data plumbing · exposure verification · the first real Statement · AI/console hardening), each with current state, what blocks it, and the cycling rule for picking work. Adopted 2026-08-09 from an outside audit plus the first live drift audit. `docs/ai-cto/context.md` points here; update a track's state in the same commit as the work that changed it.
