...
| WireGuard peers `10.8.0.4`–`10.8.0.6` | **Open** — unidentified | Present on the **live box** with real public keys, but none has a recent handshake — identify each device or remove the stale peer. The repo `wg0.conf` carries them as commented placeholder `[Peer]` blocks (real keys never in git). If kept, re-key each with `vault/rotate-secrets.sh wg-peer <name>` so an unknown holder of the old key is locked out. | [`01-core-network/wireguard/wg0.conf`](01-core-network/wireguard/wg0.conf) · [`vault/rotate-secrets.sh`](vault/rotate-secrets.sh) |
