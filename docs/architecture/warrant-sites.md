...
| Date | Change |
| ---- | ------ |
| 2026-08-08 | Loki Norn **corrected twice by the founder**, and the repeat is the finding. Draft 1 defined the office as an *auditor*, "read-only by construction" — a poltergeist that cannot trip a wire. Draft 2 made it a licensed chaos engineer with a guaranteed-useful docket. **Generator: a reflex to make any new office safe and legible**, which is § G's named error — cooling the draw instead of adding a selector — committed by the very seat meant to catch it. Landed correction: Loki *provokes* (bound by **blast radius, not verb** — wreck a disposable copy, prove the real tree clean), his sign is **genuinely unknown** and pre-judging it is forbidden, and governance moves downstream (§ G one level up: Loki is the repo's temperature, the Norns and the `*` gate are its vote). Method validated by use: nine intrusions run, seven CAUGHT, one real SHRUG converted to a wire (`check-doctrine.py` now covers `jury.py`'s docstring usage example — a reader copy-pasting the documented invocation was unguarded), one dud that turned out to be my own instrument misreading `$?` off `tail`. |
